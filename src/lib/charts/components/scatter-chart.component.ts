...
import { ScatterChartStrategy } from '../strategies';
...
export class ScatterChartComponent extends BaseChartComponent {
  ...
  private strategy: ScatterChartStrategy | null = null;

  protected createChart(): void {
    const config = this.chartConfig();
    const color = (config as any).color || '#ff7f0e';

    this.strategy = new ScatterChartStrategy(color, 4);
